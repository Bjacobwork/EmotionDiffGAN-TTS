...
from .blocks import LinearNorm, DiffusionEmbedding, Mish
from .emotion_blocks import EmotionLinear, EmotionConvNorm, EmotionMish
from .modules import FastspeechEncoder, FastspeechDecoder
from .emotion_modules import EmotionVarianceAdaptor
from .diffusion import GaussianDiffusion
from utils.tools import get_mask_from_lengths
from .loss import get_adversarial_losses_fn


class EmotionDiffGANTTS(nn.Module):
    """ DiffGAN-TTS """

    def __init__(self, args, preprocess_config, model_config, train_config):
        super(EmotionDiffGANTTS, self).__init__()
        self.model = args.model
        self.model_config = model_config

        self.text_encoder = FastspeechEncoder(model_config)
        self.variance_adaptor = EmotionVarianceAdaptor(preprocess_config, model_config, train_config)
    ...
    def forward(
        ...
        #output = self.text_encoder(emotion, texts, src_masks)
        ...
        if self.model == "naive":
            ...
            cond = output.clone()
            #coarse_mels = self.decoder(emotion, output, mel_masks)
            coarse_mels = self.decoder(output, mel_masks)
            coarse_mels = self.mel_linear(emotion, coarse_mels)
            if self.model == "aux":
...
class EmotionJCUDiscriminator(nn.Module):
    """ JCU Discriminator """

    def __init__(self, preprocess_config, model_config, train_config):
        super(EmotionJCUDiscriminator, self).__init__()
        ...
        if self.multi_speaker:
            self.spk_mlp = nn.Sequential(
                LinearNorm(hidden_size, n_channels[n_layer-1]),
            )
    ...
    def forward(self, emotion, x_ts, x_t_prevs, s, t):
        """
        x_ts -- [B, T, H]
        x_t_prevs -- [B, T, H]
        s -- [B, H]
        ...
        for layer in self.conv_block:
            x = F.leaky_relu(layer(emotion, x), 0.2)
            cond_feats.append(x)
        ...
        for layer in self.cond_conv_block:
            x_cond = F.leaky_relu(layer(emotion, x_cond), 0.2)
            cond_feats.append(x_cond)

        for layer in self.uncond_conv_block:
            x_uncond = F.leaky_relu(layer(emotion, x_uncond), 0.2)
